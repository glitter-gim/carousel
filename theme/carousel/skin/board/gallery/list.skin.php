...
<!-- 게시판 목록 시작 { -->
<div id="bo_gall" style="width:<?php echo $width; ?>" class="bo_v_wrap">
      ...
      <?php for ($i = 0; $i < count($list); $i++) {
        ...
        <li class="<?php echo implode(' ', $classes); ?>">
          <div class="gall_box">
            ...
            <div class="gall_con">
              <div class="gall_img">
                <a href="<?php echo $list[$i]['href'] ?>"> <!-- glitter 🔽 -->
                  <?php
                  if ($list[$i]['is_notice']) { // 공지사항
                    ...
                    // 썸네일이 있으면 해당 이미지, 없으면 기본 이미지 경로 사용
                    $img_src = (isset($thumb['src']) && $thumb['src']) ? $thumb['src'] : G5_THEME_URL . '/img/latest_no_image-500.png';
                    $img_alt = isset($thumb['alt']) ? $thumb['alt'] : 'no image';
                    // 이미지 태그 구성
                    $img_content = '<img src="' . $img_src . '" alt="' . $img_alt . '">';
              ...
              <div class="gall_option">
                <?php if ($is_good) { ?><span class="sound_only">추천</span><strong><i class="fa fa-thumbs-o-up"
                      aria-hidden="true"></i>
                    <?php echo $list[$i]['wr_good'] ?></strong><?php } ?>
                <?php if ($is_nogood) { ?><span class="sound_only">비추천</span><strong><i class="fa fa-thumbs-o-down"
                      aria-hidden="true"></i>
